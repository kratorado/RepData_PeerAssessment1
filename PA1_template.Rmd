...
```
The mean is `r steps_per_day.mean` and the median is `r steps_per_day.median`
...
```
The 5-minute interval `r interval_with_max_steps` contains the maximum number of steps, which is `r max_steps`
...
missing_value_count
```
The total number of missing values in the dataset is `r missing_value_count`
...
```
The mean is `r steps_per_day_new.mean` and the median is `r steps_per_day_new.median`
